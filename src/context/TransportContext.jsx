...
export function TransportProvider({ children }) {
    ...
    useEffect(() => {
        ...
        async function fetchAll() {
            try {
                const [rRes, bRes, dRes, sRes, cRes, stRes, stopsRes, bcrRes, ivRes] = await Promise.all([
                    ...
                    supabase.from('bus_change_requests').select('*').order('created_at', { ascending: false }),
                    supabase.from('industrial_visits').select('*').order('created_at', { ascending: false }),
                ]);
                let used = false;
